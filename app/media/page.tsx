      ...
      <main className="flex-1">
          ...
          <div className="container mx-auto py-8 px-4 md:px-6 lg:px-8">
            <div className="max-w-[1200px] mx-auto">
              <div className="flex justify-between items-center">
                <div>
                  <h1 className="font-gothic text-3xl md:text-4xl font-bold text-charcoal-darker mb-2 uppercase tracking-tight">
                    Media Center
                  </h1>
                  <p className="font-mono text-sm text-charcoal/80 max-w-xl">
                  ...
                  {showSearch ? (
                    <div className="relative animate-fadeIn">
                      <Input
                        ...
                        className="w-full md:w-64 font-mono text-xs pr-8 border-charcoal/10 focus-visible:ring-leafy rounded-full"
                        autoFocus
                      />
            ...
            <div className="flex overflow-x-auto scrollbar-hide gap-4 pb-4">
              {topics.map((topic) => (
                <div key={topic.id} className="flex flex-col items-center flex-shrink-0">
                  ...
                  <span className="font-mono text-xs text-charcoal/80">{topic.title}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="max-w-[1200px] mx-auto">
            <div className="flex overflow-x-auto scrollbar-hide gap-2 pb-2">
              {mediaCategories.map((category) => (
                ...
                >
                  {category.icon}
                  <span className="font-mono text-xs whitespace-nowrap">{category.name}</span>
                </button>
              ))}
            </div>
          </div>
        ...
        {(activeTab === "all" || activeTab === "videos") && (
          <section className="container mx-auto mt-8 px-4 md:px-6 lg:px-8">
            <div className="max-w-[1200px] mx-auto">
              <h2 className="font-gothic text-2xl font-bold text-charcoal-darker mb-4 uppercase tracking-tight">
                Featured Videos
              </h2>
                  ...
                  >
                    <div className="relative aspect-video w-full overflow-hidden rounded-t-xl">
                      ...
                      <div className="absolute top-0 left-0 bg-gradient-to-r from-lime-300 to-lime-500 px-3 py-1 rounded-br-lg">
                        <span className="font-mono text-xs text-forest font-medium">{video.category}</span>
                      </div>
                      <div className="absolute bottom-2 right-2 bg-black/70 px-2 py-1 rounded-md">
                        <span className="font-mono text-xs text-white flex items-center">
                          <Clock className="h-3 w-3 mr-1" /> {video.duration}
                        </span>
                      </div>
                    </div>
                    <div className="p-4">
                      <h3 className="font-gothic text-xl font-bold text-charcoal-darker group-hover:text-forest transition-colors line-clamp-2 uppercase tracking-tight">
                        {video.title}
                      </h3>
                      <div className="flex items-center mt-3">
                        <div className="flex flex-col">
                          <span className="font-mono text-xs text-charcoal/70">{video.author}</span>
                          <span className="font-mono text-xs text-charcoal/50">{video.authorTitle}</span>
                        </div>
                      </div>
                      <div className="flex items-center justify-between mt-4">
                        <div className="flex items-center gap-4 text-charcoal/60">
                          ...
                          <span className="font-mono text-xs">{video.views} views</span>
                        </div>
                        <a
                          href={video.externalUrl}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="flex items-center text-leafy hover:text-forest transition-colors font-mono text-xs"
                        >
                  ...
                  >
                    <div className="relative h-[500px] w-full">
                      ...
                      <div className="absolute top-4 left-4 bg-gradient-to-r from-lime-300 to-lime-500 px-3 py-1 rounded-full">
                        <span className="font-mono text-xs text-forest font-medium">{short.category}</span>
                      </div>

                      {/* Bottom info overlay */}
                      <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-4">
                        <h3 className="font-gothic text-lg font-bold text-white mb-2 uppercase tracking-tight">
                          {short.title}
                        </h3>
                        <div className="flex items-center mb-3">
                          <div className="flex flex-col">
                            <span className="font-mono text-xs text-white/80">{short.author}</span>
                            <span className="font-mono text-xs text-white/60">{short.authorTitle}</span>
                          </div>
                        </div>
                        <div className="flex justify-between items-center">
                          <span className="font-mono text-xs text-white/70">{short.views} views</span>
                          <a
                            href={short.externalUrl}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="flex items-center text-white hover:text-leafy transition-colors font-mono text-xs"
                          >
                            Watch <ExternalLink className="h-3 w-3 ml-1" />
                          </a>
                        </div>
        ...
        <section className="container mx-auto mt-8 mb-16 px-4 md:px-6 lg:px-8">
          <div className="max-w-[1200px] mx-auto">
                    ...
                    <div className="relative h-full w-full">
                      ...
                      <div className="absolute inset-0 bg-black/30 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                        <div className="text-white text-center p-4">
                          <h3 className="font-gothic text-sm font-bold mb-2 line-clamp-2 uppercase tracking-tight">
                            {item.title}
                          </h3>
                      ...
                      <div className="absolute bottom-2 left-2 bg-gradient-to-r from-forest/90 to-leafy/70 px-2 py-0.5 rounded-md">
                        <span className="font-mono text-[10px] text-white">{item.category}</span>
                      </div>

                      {/* Duration (if applicable) */}
                      {item.duration && (
                        <div className="absolute bottom-2 right-2 bg-black/70 px-1.5 py-0.5 rounded-md">
                          <span className="font-mono text-[10px] text-white flex items-center">
            ...
            <div className="flex justify-center mt-8">
              <Button className="bg-gradient-to-r from-lime-300 to-lime-500 hover:from-lime-400 hover:to-lime-600 text-forest-dark font-medium font-mono text-xs rounded-full transition-all duration-300 transform hover:-translate-y-0.5 shadow-sm border border-lime-400/20 hover:shadow-md">
